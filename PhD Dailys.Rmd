...
household_data$uuid <- gsub("uuid:", "", household_data$instanceID)
...
household_data$surveyor_name[household_data$uuid == "e6c9f82c-7257-41e1-90f8-0f1df5df1741"] <- "Musa"
household_data$surveyor_name[household_data$uuid == "20bbff41-5fa0-474d-874a-0d67251e3005"] <- "Musa"

household_variables <- c("uuid", "community", "hhid", "surveyor_name", "survey_date", "start_time", "end_time", "gps_location")

household_data_current_month <- household_data %>%
  select(all_of(household_variables)) %>%
  filter(survey_date >= floor_date(Sys.Date(), "month"))
household_data_previous_month <- household_data %>%
  select(all_of(household_variables)) %>%
  filter(survey_date >= floor_date(Sys.Date() - months(1), "month"),
         survey_date < floor_date(Sys.Date(), "month"))
household_data_all <- household_data %>%
  select(uuid, hhid, community, surveyor_name, survey_date, start_time, end_time, gps_location)
...
source_variables <- c("community", "surveyor_name", "survey_date", "start_time", "end_time", "gps_location", "consent")

source_data_current_month <- source_data %>%
  select(all_of(source_variables)) %>%
  mutate(date_label = format(survey_date, "%b %d")) %>%
  filter(survey_date >= floor_date(Sys.Date(), "month"))
source_data_all <- source_data %>%
  select(all_of(source_variables))
source_data_two_months_ago <- source_data %>%
  select(all_of(source_variables)) %>%
  filter(survey_date >= floor_date(Sys.Date() - months(2), "month"),
         survey_date < floor_date(Sys.Date() - months(1), "month"))
```
...
completed <- household_data_current_month %>%
...
**Total**: `r sum(community_progress$completed)`/`r sum(community_progress$target)` surveys completed, `r sum(community_progress$remaining)` remaining
...
all_dates <- sort(unique(c(household_data_current_month$survey_date, source_data_current_month$survey_date)), decreasing = TRUE)

household_times <- household_data_current_month %>%
...
source_times <- source_data_current_month %>%
  group_by(surveyor_name, survey_date) %>%
  summarise(
    start = min(start_time),
    end = max(end_time),
...
week_totals <- household_data_current_month %>%
  filter(survey_date >= Sys.Date() - 7) %>%
  count(surveyor_name, name = "hh_week") %>%
  full_join(
    source_data_current_month %>%
      filter(survey_date >= Sys.Date() - 7) %>%
...
month_totals <- household_data_current_month %>%
...
# Grand total
grand_daily <- household_data_current_month %>%
...
grand_week <- household_data_current_month %>%
  filter(survey_date >= Sys.Date() - 7) %>%
  summarise(hh_week = n()) %>%
  bind_cols(
    source_data_current_month %>%
...
grand_month <- household_data_current_month %>%
  filter(survey_date >= Sys.Date() - 30) %>%
...
portee_daily <- household_data_current_month %>%
...
portee_week <- household_data_current_month %>%
...
portee_month <- household_data_current_month %>%
...
# Dworzark subtotal
dworzark_daily <- household_data_current_month %>%
...
dworzark_week <- household_data_current_month %>%
...
dworzark_month <- household_data_current_month %>%
...
# Cockle Bay subtotal
cockle_daily <- household_data_current_month %>%
...
cockle_week <- household_data_current_month %>%
...
cockle_month <- household_data_current_month %>%
...
## Portee Rokupa - Remaining Households Map
```{r portee-remaining-map, eval=F}
# Read the shapefile
...
# Get unique hhids from previous month in Portee Rokupa
previous_hhids <- household_data_previous_month %>%
  filter(community == "Portee Rokupa") %>%
  pull(hhid) %>%
  unique()

# Get unique hhids from current month in Portee Rokupa
current_hhids <- household_data_current_month %>%
  filter(community == "Portee Rokupa") %>%
  pull(hhid) %>%
  unique()

# Find hhids that still need to be surveyed
remaining_hhids <- setdiff(previous_hhids, current_hhids)

# Get location data for remaining households (take first occurrence per hhid)
portee_remaining <- household_data_previous_month %>%
  filter(community == "Portee Rokupa") %>%
  filter(hhid %in% remaining_hhids) %>%
...
portee_remaining_utm <- st_transform(portee_remaining, crs = 32628)
portee_shapefile_utm <- st_transform(portee_shapefile_wgs84, crs = 32628)

# Filter households within 100m of community boundary
distances <- st_distance(portee_remaining_utm, portee_shapefile_utm)
portee_remaining_filtered <- portee_remaining[as.numeric(distances) <= 100, ]

# Count statistics
n_previous <- length(previous_hhids)
n_completed <- length(current_hhids)
n_remaining <- length(remaining_hhids)

# Plot
ggplot() +
  annotation_map_tile(type = "osm", zoomin = 0, progress = "none") +
  geom_sf(data = portee_shapefile_wgs84, fill = NA, color = "black", linewidth = 1) +
  geom_sf(data = portee_remaining_filtered, 
          color = "red", fill = "yellow", shape = 21, size = 3, stroke = 1) +
  labs(
    title = "Portee Rokupa - Remaining Households to Survey",
    subtitle = paste0("Previous month: ", n_previous, " | Completed this month: ", n_completed, " | Remaining: ", n_remaining)
  ) +
  theme_minimal() +
  theme(
...
dworzark_households <- household_data_current_month %>% 
  filter(community == "Dworzark") %>%
...
dworzark_sources <- source_data_current_month %>% 
  filter(community == "Dworzark") %>%
  select(community, survey_date, surveyor_name, gps_location) %>%
...
household_data_current_month <- household_data_current_month %>%
  filter(month(survey_date) == current_month & year(survey_date) == current_year)

source_data <- source_data %>%
  filter(month(survey_date) == current_month & year(survey_date) == current_year)

# Get all unique dates (now only from current month)
all_dates <- sort(unique(c(household_data_current_month$survey_date, source_data$survey_date)), decreasing = T)
