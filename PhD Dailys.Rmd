...
combined_times <- household_times %>%
  ...
  mutate(time_range = paste0(format(start, "%H:%M"), " - ", format(end, "%H:%M"), " (", n_hh, " / ", n_source, ")")) %>%
  select(surveyor_name, survey_date, time_range) %>%
  complete(surveyor_name, survey_date = last_7_days, fill = list(time_range = "")) %>%
...
combined_times %>%
  kable(col.names = c("Surveyor", format(last_7_days, "%a %b %d"))) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
